...
@Component("httpGetRestClient")
public class HTTPGetRestClientImpl implements HTTPRestClient<ResponseDO>{
	...
	public ResponseDO makeWSCall(String restURI
		...
		try {
			...
			getRequest = new HttpGet(restURI + resourceURIPath);
			...
			if(responseJson.toString().trim().length() >0){
				...
				reponseDO.setJsonReponse("*** No Payload Received In Response Body ***");
			}
			
			reponseDO.setHttpMethodType(HTTPMethodType.GET);
			
		} catch (ClientProtocolException clientProtocolException) {

			throw new ApplicationException(" HTTPGetRestClientImpl :: makeWSCall :: MalformedURLException Occured : ", clientProtocolException);

		} catch (IOException ioException) {

			throw new ApplicationException(" HTTPGetRestClientImpl :: makeWSCall :: MalformedURLException Occured : ", ioException);
		}finally{
