...
@Component("httpPostRestClient")
public class HTTPPostRestClientImpl implements HTTPRestClient<ResponseDO>{
	...
	public ResponseDO makeWSCall(String restURI
		...
		try {
			...
			postRequest = new HttpPost(restURI + resourceURIPath);
			
			if(null != inputJSON){
				postRequest.setEntity(new StringEntity(inputJSON, "UTF-8"));
			}

			postRequest.addHeader("Accept", consumesMediaType.toString());
			...
			reponseDO.setHttpMethodType(HTTPMethodType.POST);
			
		} catch (ClientProtocolException clientProtocolException) {

			throw new ApplicationException(" HTTPPostRestClientImpl :: makeWSCall :: MalformedURLException Occured : ", clientProtocolException);

		} catch (IOException ioException) {

			throw new ApplicationException(" HTTPPostRestClientImpl :: makeWSCall :: MalformedURLException Occured : ", ioException);
		}finally{
